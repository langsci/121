...
\ea
{Announcer speaks in \ili{English}; Caller speaks in \ili{English} with an unmistakably \ili{Luo} accent}\\
...
\ea\label{ex:muaka:5}
{\ili{English}, with Nigerian portrayed as speaking in a heavy Nigerian-\ili{English} accent}\\
...
once! “The mobile subscriber cannot be found!” \textup{ [In a heavy Nigerian \ili{English} accent]. [Laughter].}\\
Try calling again. “Hello, didn’t I just tell you the mobile subscriber cannot be found? \\
\textup{[Laughter]} What is your problem, stupid man?” \textup{[In a heavy Nigerian \ili{English} accent].} \citep{Omogi2012}
\z
...
{\ili{Swahili} and \ili{English}, with some Kikuyu}\\
   Ujaluo utakuua-\\
...
\ea
 People from \ili{Nyanza} like prefixes. A prefix is a title that comes before the name: Dr Geofrey Otieno, Engineer Obado, Lecturer Omondi. People from Central also like something called suffixes. Suffix is a title that comes after the name: Mwangi wa Equity, Wanjiku wa makara.\\
...
‘You know a \ili{Luhya} can curse you but the way the curse is “packaged” you won’t realize you have been cursed. A \ili{Luhya} can say to you, “Boss, so you made it here, thank you for coming to the show… [high five]’ \citep{ChurchillRaw2014}
\z

The wordplay here is the twist that delivers the humor to an audience that shares in the language stereotypes. There is a tendency for \ili{Luhya} dialects to add a particle or suffix like \textit{ko} and \textit{nga} to a conjugated verb. When \textit{ko} is added to the verb \textit{kamata}, the new word created could be a dimunitive for ‘butt’. Consequently in Omwami’s joke, the high five could also mean ‘you little butt…’
...
In the next example, Majimoto this time pokes fun at his community that is often considered less educated and limited in its knowledge of \ili{English}. As a coastal resident, he pokes fun at his own Mombasa people because he is one of them. This will not be an easy joke for an outsider to make considering that the people from this area are limited in their \ili{English} proficiency due to marginalization and unfair allocation of public resources. Moreover, this is supposedly a joke about his own mother. What the audience finds funny is the misinterpretation of the phrase \textit{high class} by the mother. \textit{High class} is used here to mean ‘sophisticated /classy’ but the mother understands it to mean an upper grade in school. 
...
As we have seen in the examples above, most artists choose to poke fun at themselves or at their own ethnic groups instead of laughing at other groups. \ili{Luo} comedians such as Omondi and Owago make fun of \ili{Luo} practices, while \ili{Luhya} comedians like Omwami freely joke about their \ili{Luhya} follies. Instead of outsiders poking fun at the \ili{Luhya} or \ili{Luo} accents in spoken \ili{English}, the comedians from those groups take it upon themselves to bring this out in the jokes. Owago, for example, discusses the issue of “shrubbing” among Luos. \textit{Shrubbing} (or \textit{srub}) is a \ili{Sheng} word which refers to the unintentional [${\int}$]/ [s] substitution characteristic of many \ili{Luo} speakers of \ili{English} or \ili{Swahili} as second languages.

\ea
{\ili{English} with \ili{Sheng} code-mixing (in bold)}\\
...
{\ili{English} with \ili{Swahili} code mixing (in bold)}\\
...
\ea
{\ili{Swahili} with \ili{English} code-mixing}\\
