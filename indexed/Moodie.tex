...
In many languages, a distinction can be made between countable and non-countable nouns. Non-countable nouns cannot be differentiated on the basis of number. They are called mass nouns. In \ili{Lopit}, mass nouns are inherently either singular or plural.  The words for ‘milk’ and ‘water’ take plural agreement, whereas those for ‘air’, ‘grass’ and ‘flour’ take singular agreement. Examples are shown in \REF{ex:moodie:1} and \REF{ex:moodie:2}. In \ili{Lopit}, relative clause pronouns inflect for the gender and number of the noun they modify.\footnote{\ili{Lopit} is a tone language with high, low and falling tones. It has ATR distinctions which are not substantive in relation to number marking and are ignored in this paper. In general the \ili{Latin} script is used as in \ili{English} except that the sounds /x/, /ɲ/ and /ŋ/ are represented by <h>, <ny> and <ng> respectively.}
...
\multicolumn{2}{c}{ {Plurative}} & \multicolumn{2}{c}{ {Singulative}} & \multicolumn{3}{c}{ {Replacement}}\\
\cmidrule(lr){1-2}\cmidrule(lr){3-4}\cmidrule(lr){5-7}
 {Plural} & {Number} & {Singular} & {Number} & {Singular} & {Plural} & {Number}\\ \midrule
 -i & 73 &  -i & 30 &  -ni &  -k & 18\\
 -hi & 5 &  -ti & 34 &  hi- &  -i & 13\\
...
To explore what suffix choices might be regularized, three consultants were tested on their intuitions about the plural marking of loan words from \ili{English} and \ili{Arabic} and of nonce words.\footnote{A nonce word is one which has been made up for testing purposes. Since consonant clusters are not normally found in \ili{Lopit} words, the testing was based on CV(CV)(C) words. Sometimes, consultants also gave \ili{Arabic} plurals e.g.\ \textit{kabaya}, \textit{kabaya-t} and these have been ignored.} A smaller sample of 66 words was used and several patterns were identified. The syllable structure of the stem appears to play a role. The results are shown in \tabref{tab:moodie:13} together with some examples.
...
\lsptoprule
 ...
 {Semantics} & {Stem} & {Singular} & {Plural} & {English}\\ \midrule
...
It appears that \ili{Sila} has lost the morphological unmarked form of \textit{nyir}, ‘teeth’. 
