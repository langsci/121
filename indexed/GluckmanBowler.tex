...
Based on the answers to these two questions, we suggest that \ili{Luragooli} \textit{-Vk} should be analyzed as an anticausative suffix. That is, \textit{-Vk} can be treated as a marker of intransitivity analogous to the \ili{English} anticausative in \REF{ex:gluckman:1b} below. While in \ili{English} there is no morphological difference between the causative and anticausative forms of the verb \textit{break} (cf. \ref{ex:gluckman:1a} and \ref{ex:gluckman:1b}), in \ili{Luragooli} the difference between the two is marked by presence versus absence of the –V\textit{k} suffix, as in (\ref{ex:gluckman:2}a-b):
...
\ea\label{ex:gluckman:1a} 
 ...
 \z
\z
\todo{here use jambox and small caps labels to right of exs?}
\ea\label{ex:gluckman:2}{Luragooli}{}{}\\
\ea \label{ex:gluckman:2a}
 
\gll  \ili{Sira} a-han-i       muriaŋgo.~ ~ ~ ~     \textsc{(lexical) causative}\\
       \ili{Sira} 1-close-\textsc{fv} 3door\\
...
\begin{table}
\caption{Intransitive verb classes in \ili{Luragooli} based on the distribution of -Vk}
\label{tab:gluckman:1}

\fittable{
\begin{tabular}{p{32mm}p{36mm}p{46mm}}
\lsptoprule
\textbf{Class I}\newline (\textit{\small intransitive with -Vk}) &
\textbf{Class II}\newline (\textit{\small intransitive without -Vk}) & 
\textbf{Class III}\newline (\textit{\small intransitive with or without -Vk})\\
\midrule
...
The following three sub-sections show how oblique theta-roles combine with the three types of intransitives in \ili{Luragooli}: passives, plain intransitives, and \textit{-Vk} intransitives. The plain and \textit{-Vk} intransitives pattern similarly with respect to the theta-roles they license in oblique phrases, as we expect from anticausatives.
...
In \sectref{sec:gluckman:3} and \ref{sec:gluckman:4} we provided evidence that \ili{Luragooli} \textit{-Vk} intransitives generally pattern distinctly from the passive and the plain intransitive forms in terms of theta-roles and lexical aspect. The accumulated evidence led us to conclude that \textit{-Vk} is the anticausative marker in \ili{Luragooli}. Our conclusion was based largely on a comparison with cross-linguistic observations. In this section, we detail some “anomalous” uses of \textit{-Vk} that fall outside of what is typically associated with an anticausative alternation cross-linguistically.
...
\lspbottomrule
\end{tabularx}
\textsuperscript{a} In \ili{Luragooli}, ‘to hear’ and ‘to punch’ are entirely homophonous (\textit{kuhola}), with no tonal differences.
...
\ili{Luragooli} has 20 noun classes. Following Bantuist convention, we mark noun classes through numerals at the beginning of nouns and verbs. 
