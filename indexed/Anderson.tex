...
\lsptoprule
\parbox{.9\textwidth}{
\textit{\ili{Atlantic-Congo} families, isolates and stocks}\\
\begin{tabular}{llll}
% \hline
...
\textit{Central \ili{Sudanic} families}\\
\begin{tabular}{lll}
\textsc{Kresh} & \textsc{Lendu} & \textsc{Mangbetu}\\
\textsc{Mangbutu-Lese/Efe} & \textsc{Moru-Ma'di} & \textsc{Sara-\ili{Bongo}-Bagirmi}\\
\end{tabular}

\bigskip


\textit{\ili{Mande} families}\\
\begin{tabular}{llll}
...
\begin{tabular}{llll}
\textsc{Gbaya} & \textsc{Ngbandi} & \textsc{Mba} & \textsc{Zande}(+traces in \textsc{Ngbaka, \ili{Banda})}\\
\end{tabular}
}
\\
\lspbottomrule
\end{tabular}
% \todo[inline]{Table missing top and bottom lines. Omit line under "\ili{Atlantic-Congo}..."}
\end{table}

\section{Functions of STAMP morphs}\label{sec:Anderson:3}

STAMP morphs usually combine with a following verb to encode the TAM categories of the event, and the person, number and, where relevant, gender categories of the subject. Such forms can be found in as diverse array of languages of the \ili{Macro-Sudan} Belt as the \textsc{Biu-Mandara (Central) Chadic} language \ili{Merey} \REF{ex:anderson:3} of Cameroon, the \textsc{Kru} language \ili{Wobé} \REF{ex:anderson:4} of Côte d'Ivoire, and \ili{Bongo} of the \textsc{Sara-\ili{Bongo}-Bagirmi} family of Central \ili{Sudanic} \REF{ex:anderson:5} spoken in South Sudan.

\ea\label{ex:anderson:3}
\ili{Merey}  \citep[8]{Gravina2007}                  \textsc{[Biu-Mandara \ili{Chadic}]}\\
\gll na  zal    \\
\textsc{1.pst}  call   \\
\glt `I called.'  
\z
...
\ili{Wobé} \citep[\ili{Wobé} 3]{HoferLink1973}\footnote{Hofer \& Link's volume does not number the chapters consecutively, but rather each chapter begins anew at page 1. Thus this form comes from page 3 of the \ili{Wobé} chapter. Their page-numbering convention is followed here.}    \textsc{[Kru]}\\
...
\ili{Bongo}   \citep[75]{TuckerBryan1966}       [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
\gll ma    bi\\
1.\textsc{indef}  give  \\
\glt `I am giving.'
\z

One of the curious and characteristic features of STAMP morph constructions is the possible encoding of negative polarity without any distinct negative polarity scope operator, the negative polarity being encoded together with TAM categories and referent properties of the subject in the STAMP morph itself. Formations of this type can be found in languages of the \ili{Macro-Sudan} Belt such as \ili{Duka} (\textsc{Kainji}) \REF{ex:anderson:6} or \ili{Mano} (\textsc{Southeast Mande}) \REF{ex:anderson:7}. 

\ea\label{ex:anderson:6}
\ili{Duka}  \citep[13]{BendorSamuelEtAl1973}      [\textsc{Kainji}]\\
\gll mân  hé  ò-k\'ɔt  \\
...
\ili{Mano} \citep[226]{Vydrine2009}           [\textsc{Southeast Mande}]\\
\gll l\`{ɛ}\'{ɛ}      máá    wè    gè\={e}\\
\textsc{3sg.neg[hab]}  \ili{Mano}    language  speak\\
\glt `S/he doesn't speak \ili{Mano}.'
\z

As noted earlier, the referent categories expressed in STAMP morphs are typically restricted to subjects, but in a small number of instances one also finds portmanteau subject > object forms within a STAMP morph, as in the following sentence from \ili{Kohumono} (\textsc{Cross River}).

\ea\label{ex:anderson:8}
 \ili{Kohumono} \citep[355]{Cook1972}         [\textsc{Cross River}]\\
\gll {β\'ɔ}    fà\\
1>2.\textsc{npst}  bite\\
\glt `I bite you.'
\z

In \ili{Guro} (\textsc{Southeast Mande}), one finds portmanteau subject > object-encoding STAMP morphs that also reference polarity, as in \textit{ɓ}\textit{e} vs. \textit{yaa} in the following sentences (note the tone differences on the verbs in these constructions as well as the presence of the postverbal negator \textit{ɗo} in \REF{ex:anderson:9b}). 

\ea\label{ex:anderson:9}
\ea\label{ex:anderson:9a}
\ili{Guro} \citep[239]{Vydrine2009}       [\textsc{Southeast} \textsc{Mande}]\\
\gll  ɓe      zuru-o      \\
...
\ili{Fyem} \citep[32, 35]{Nettle1998}          \textsc{[Plateau]}\\
...
The \textsc{Waja-Kam} language \ili{Dadiya} shows similar constructions.

\ea\label{ex:anderson:15}
\ili{Dadiya}   \citep[196]{\ili{Ju|’hoan}ngraithmayr1968}       [\textsc{Waja-Kam]}  \\
\gll \'{n}    já    \\
...
Such STAMP morph constructions are found in \ili{Tiv} (\textsc{Bantoid}) \REF{ex:anderson:17}, and Ndut-\ili{Falor} (\textsc{Cangin}) \REF{ex:anderson:18}.

\ea\label{ex:anderson:17}
\ea\label{ex:anderson:17a}
\ili{Tiv} \citep[114]{Arnott1958}          \textsc{[Bantoid/Tivoid]}\\
\gll vé     pìnè        \\
...
Ndut-\ili{Falor} \citep[Ndut-\ili{Falor} 4]{Pichl1973}      [\textsc{Cangin}]\\
\gll mi    acɛ          \\
...
This is found in the following formation from \ili{Dadiya} of the \textsc{Waja-Kam} genetic unit \REF{ex:anderson:20}:

\ea\label{ex:anderson:20}
\ili{Dadiya}   \citep[197]{\ili{Ju|’hoan}ngraithmayr1968}        [\textsc{Waja-Kam}]\\
...
\ea\label{ex:anderson:25}
 \ili{Duka}  \citep[96-98; 105]{BendorSamuelEtAl1973}    \textsc{[Kainji]}\\
 \gll m\'{ɛ}    he\'{ɛ}   \\
I.\textsc{irr}  go:\textsc{cond}  \\
\glt `If I go...'  
\z

Split negative marking is relatively common in AVCs (Anderson 2006; 2011) where the negative is encoded on a lexical verb but other obligatory categories are on the auxiliary. In Africa, this pattern is a feature of auxiliary verb constructions in the languages of the \textsc{\ili{Rashad} Kordofanian} genetic unit, e.g. in \ili{Rashad} proper \REF{ex:anderson:26}.

\ea\label{ex:anderson:26}
\ili{Rashad} \citep[297]{TuckerBryan1966}      [\textsc{\ili{Rashad} Kordofanian}]\\
...
Such a formation in the negative future in \ili{Gã} of the \textsc{Ga-Dangme Kwa} genetic unit \REF{ex:anderson:28}.\footnote{Negative can be encoded in the STAMP morph itself as well. When so, the verb is thus in a `co-negative' form.}
...
\ili{Gidar} \citep[263]{Frajzyngier2008}        \textsc{[Biu-Mandara \ili{Chadic}]}\\
...
Such a formation may be found in a small number of \ili{Chadic} languages, for example, \textsc{West Chadic} Polci \REF{ex:anderson:31} or \textsc{Biu-Mandara (Central) Chadic} \ili{Mofu-Gudur} \REF{ex:anderson:31}. 

\ea\label{ex:anderson:31}
Polci \citep[153]{Caron2008}          \textsc{[West \ili{Chadic}]}\\
...
\ili{Mofu-Gudur} \citep[4]{Pohlig1992}          \textsc{[Biu-Mandara \ili{Chadic}]}\\
\gll fá    tá-ka    ɗáf      \\
...
\ili{Limbum} \citep[\ili{Limbum} 3-4]{FiorePeck1973/1980}      \textsc{[Bantoid]}\\
...
Researchers on individual languages and families have recognized this connection. Thus \citet[101]{Shimizu1983}, when describing STAMP morphs in the \ili{Leko-Nimbari} language \ili{Zing} \ili{Mumuye} states that ``the surface differences in subject pronouns are in fact due to the TAM markers, which are realized on them or contracted with them.'' Further, \citet[35]{Babaev2010}, in discussing issues in the reconstruction of \ili{Benue}-Congo cautions the reader that ``… various phonological processes of merging person markers with predicative markers of tense, aspect, modality and polarity have made the situation in many languages obscure.''
  ...
  Language-specific internal evidence sheds light on this most likely source for the development of STAMP morphs. In Sara of the \textsc{Sara-\ili{Bongo}-Bagirmi} family of the Central \ili{Sudanic} stock, the so-called indefinite series subject pronouns are not used with finite, inflected forms of a following vowel-initial verb, but rather with an infinitive form.
  
  \ea\label{ex:anderson:34}
  Sara \citep[75]{TuckerBryan1966}      [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
\gll ma    k-usa\\
...
\ili{Nomaande} \citep[22]{Wilkendorf2001}          \textsc{[Bantoid]}\\
\gll y\'{ɔ}    ɔcɔba  ná-áyá\\
3.\textsc{prs}  \textsc{inf}:go  to-\textsc{3sg}\\
\glt `He is going to him (i.e., his home).' 
\z

Similarly, in Dott (also known as \ili{Dass} or \ili{Zoɗi}) of the \textsc{West Chadic} family \REF{ex:anderson:36}, the continuous or progressive STAMP morph series requires a verbal noun form of the lexical verb in the predicate. 

\ea\label{ex:anderson:36}
Dott/\ili{Zoɗi} \citep[165]{Caron2002}            [W\textsc{est} \textsc{Chadic}]\\
\gll taa    tʃét-ti\\
\textsc{3pl.cont}  come-\textsc{vn}\\
\glt `They are coming.'
\z

Yet other languages of the \ili{Macro-Sudan} Belt require dependent-marked forms of verbs to appear in combination with STAMP morphs in specific constructional subtypes. Such languages include the \textsc{Kainji} language \ili{Duka} \REF{ex:anderson:37} and 'Bozom \REF{ex:anderson:38} of the \textsc{Gbaya Ubangi} family. 
...
`Bozom \citep[159]{Monino1995}            \textsc{[Gbaya \ili{Ubangi}]}\\
\gll ʔà̰    rè-á      \\
...
\ili{Banda} Nchumuru \citep[Nuchumuru 4]{Cleal1973}      [\textsc{Potou-Tano} K\textsc{wa}]\\
\gll mà-ba\\
...
Gerka  \citep[173]{\ili{Ju|’hoan}ngraithmayr1968}\\
\gll kà  tà  ƒàm\\
...
\ili{Merey} (also \textsc{Biu-Mandara Chadic}) shows an unusual intra-pardigmatic split between free-standing forms in the first singular past but bound prefix elements in the corresponding third  person \REF{ex:anderson:45} (masculine singular) past form. 

\ea\label{ex:anderson:45}
\ili{Merey}  \citep[8]{Gravina2007}          [\textsc{Biu-Mandara Chadic}]\\
...
\ili{Buduma} \citep[376]{Pawlak2001}, \citep[55]{Lukas1939}    [\textsc{Biu-Mandara Chadic}]\\
\gll a-kol  a  jai-ni\\
\textsc{3.prs}-be  at  sit\textsc{-vn}\\
\glt `He is/was sitting.' 
\z

While prefix conjugations are archaic features of Afroasiatic languages \citep{Hodge1971, Schuh1976, Mukarovsky1983, Voigt1987}, the ones attested in \ili{Chadic} languages do not reflect inherited structures \citep{Schuh1976, Voigt1989, \ili{Ju|’hoan}ngraithmayr2005, \ili{Ju|’hoan}ngraithmayr2006}, but rather reflect secondary developments typically derived from the the univerbation of STAMP morphs with following verbs \citep{Caron2006, Shay2008, Anderson2011, Anderson2012}.

\section{STAMP morphs and conjugational prefixes in Central \ili{Sudanic} languages}\label{sec:Anderson:8}

STAMP morphs and prefixed conjugation series derived from STAMP morphs are also found in the various families of the Central \ili{Sudanic} stock \citep{Anderson2015}. Within the analytic tradition of \ili{Ma'di} \REF{ex:anderson:47}, a member of the \textsc{Moru-Ma'di} genetic unit \citet{BlackingsFabb2003} analyze the freestanding STAMP morph \textit{ka}/\textit{k\'ɔ} as a pronominal, but \citet{TuckerBryan1966} analyze it as an auxiliary. Such disagreement over what traditional part-of-speech to assign these complexes to underscores the unusual but characteristic features of STAMP morphs. 

\ea\label{ex:anderson:47}
\ili{Ma'di} \citep[13]{BlackingsFabb2003}      [\textsc{Moru-Ma'di}]\\
...
Various \textsc{Sara-\ili{Bongo}-Bagirmi} languages require infinitive forms of vowel-initial verbs when used with freestanding STAMP morphs, for example, Ngambay \REF{ex:anderson:48}, Kabba \REF{ex:anderson:49} and \ili{Kenga} \REF{ex:anderson:50}.

\ea\label{ex:anderson:48}
Ngambay \citep[118]{Vandame1963}      [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
...
Kabba  \citep[220]{Moser2004}         [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
...
\ili{Kenga}  \citep[15]{Neukom2010}        [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
\gll \={m}    k-\`{ɔ}s\={ɔ}\\
1.\textsc{fut}  \textsc{inf}-eat\\
\glt `I will eat.'
\z

As with \ili{Chadic}, many languages in different sub-families of Central \ili{Sudanic} have a bound `definite' perfective STAMP series that contrasts with an unbound `indefinite' imperfective series. The pattern is found in \ili{Bongo} \REF{ex:anderson:51}, \ili{Lugbara} \REF{ex:anderson:52}, Lendu \REF{ex:anderson:53} and \ili{Oke'bu} \REF{ex:anderson:54}, from four different genetic units of Central \ili{Sudanic}. 

\ea\label{ex:anderson:51}
\ili{Bongo}   \textit{*mi- < m-i} \citep[75]{TuckerBryan1966}    [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
...
\ili{Lugbara} \citep[47]{TuckerBryan1966}         \textsc{[Moru-\ili{Ma'di}]} \\
...
\ili{Oke'bu} \citep[48]{TuckerBryan1966}        \textsc{[Mangbutu]}\\
...
As the reader may have noticed, there is also a difference in the basic clausal syntax of the two series in these Central \ili{Sudanic} languages, with VO order in the definite/perfective series and OV in the indefinite/imperfective one. The VO order in the definite series vs. OV order in the indefinite is, however, not universal in Central \ili{Sudanic} and is not found, for example, in \ili{Baka} \REF{ex:anderson:55} of the \textsc{Sara-\ili{Bongo}-Bagirmi} family or Kresh \REF{ex:anderson:56}.
...
Note that these two languages however also have infinitive forms of the verb with vowel initial stems with the indefinite series STAMP morph, like those in \REF{ex:anderson:48}-\REF{ex:anderson:50}. This feature further underscores the likely origin of STAMP morph in an auxiliary verb construction in Central \ili{Sudanic} languages.

Of course, once a bound STAMP morph prefix exists in a given Central \ili{Sudanic} language, it is free to attach to auxiliary verbs in new AVCs, as seen in the following \ili{Lugbara} sentence \REF{ex:anderson:57}.

\ea\label{ex:anderson:57}
\ili{Lugbara} \citep[46, 47]{TuckerBryan1966}      \textsc{[Moru-\ili{Ma'di}]}\\
\gll ma-ŋga  mvá  tsɔ\\
\textsc{1.def-aux}  child  beat\\
\glt `I shall beat the child.'    
\z

Variation in closely related varieties between bound and unbound formations can be seen in \textsc{Sara-\ili{Bongo}-Bagirmi} languages. Compare in this regard \ili{Furu} \REF{ex:anderson:58}, where a freestanding STAMP morph is found, with the corresponding sentence in its close sister language \ili{Bagiro} \REF{ex:anderson:59}, where it is a prefix.

\ea\label{ex:anderson:58}
\ili{Furu} \citep[91]{Boyeldieu1990}           [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
\gll mí  gáli  gɔ          \\
I   know  \textsc{neg}      \\
\glt `I don't know.'
\z
...
\ili{Bagiro}  \citep[91]{Boyeldieu1990}           [\textsc{Sara-\ili{Bongo}-Bagirmi}]\\
...
\ili{Mangbetu} \citep[106]{Larochette1958}        \textsc{[\ili{Mangbetu}]}\\
...
Meje \citep[106]{Larochette1958}           \textsc{[\ili{Mangbetu}]}\\
...
\ili{Mangbetu} \citep[106-7]{Larochette1958}        \textsc{[\ili{Mangbetu}]}\\
\gll mú-ta\\
...
Meje \citep[106-7]{Larochette1958}        \textsc{[\ili{Mangbetu}]}\\
\gll ú-ta\\
\textsc{prs/fut}-carry\\
\glt `You (will) carry.' 
\z

The \textsc{Mangbutu} language Lese also has only bound prefix conjugations. Again these appear to reflect two originally distinct series, such as the present progressive in the first singular in \textit{má}- \REF{ex:anderson:64} and the future progressive first singular in \textit{m\'ʊ-} \REF{ex:anderson:65}.
...
\section{STAMP morphs and conjugational prefixes in \ili{Niger-Congo} languages}\label{sec:Anderson:9}

STAMP morphs and conjugational prefixes derived from STAMP morphs are found in a wide range of \ili{Niger-Congo} languages. These include \ili{Klao} of the \textsc{Kru} family \REF{ex:anderson:66}, \ili{Dadiya} of the \textsc{Waja-Kam} family \REF{ex:anderson:67}, \ili{Gã} of the \textsc{Ga-Dangme Kwa} genetic unit \REF{ex:anderson:68}, and \ili{Kulango} \REF{ex:anderson:69}.

\ea\label{ex:anderson:66}
\ili{Klao} \citep[3, 18]{Marchese1982}            [\textsc{Kru}]\\
\gll \={ɔ}\={ɔ}    bl\={e}  \\
he\textsc{:ipfv}   sing\\
\glt `He is singing.'
\z
...
Ga \citep[105]{Kropp1988}          \textsc{[Ga-Dangme \ili{Kwa}]}  \\
\gll èe     n\`{\~u}  n\`{\~u}  \\
...
\ili{Kulango} \citep[193]{Elders2007}            \textsc{[\ili{Kulango}]}\\
...
Such formations are also widespread in \ili{Mande} languages, such as the following example from \ili{Jo}(wulu) of the \textsc{Northwest Mande} genetic unit \REF{ex:anderson:70}.

\ea\label{ex:anderson:70}
\ili{Jo} (Jowulu) \citep[11]{Kim2002}          [\textsc{Northwest Mande}]  \\
...
Evidence for the *\textit{ma-} ({\textasciitilde}\textbf{*}\textit{mà}-) perfective/realis/past series in \ili{Benue}-Congo \REF{ex:anderson:71} comes from a range of different sub-groups of the stock. Thus one finds formally and functionally cognate elements in such \ili{Benue}-Congo languages as \textsc{Bantoid} \ili{Ndemli} \REF{ex:anderson:72}, Eleme of the \textsc{Cross River} family \REF{ex:anderson:73}, and \ili{Berom} of the \textsc{Plateau} sub-stock \REF{ex:anderson:74}.

\ea\label{ex:anderson:72}
\ili{Ndemli}   \citep[72]{Ngoran1999}            [\textsc{Bantoid}]\\
\gll mà-tóm \\
\textsc{I.pst-}send\\
\glt  `I sent.'   
\z
...
\ili{Berom}  \citep[299, 301]{Bouquiaux1970}          [\textsc{Plateau}]\\
\gll mà-ciŋ\\
1.\textsc{pst/prf}-dig\\
\glt `I (have) dug.'
\z

Evidence for the high-toned irrealis or future series marked in the first singular by \textit{*mé} can be found in a range of \ili{Benue}-Congo languages, e.g., the \textsc{Bantoid} language \ili{Tiv} \REF{ex:anderson:75}, the \textsc{Kainji} language \ili{Duka} \REF{ex:anderson:76} or \ili{Berom} of the \textsc{Plateau} stock \REF{ex:anderson:77}.

\ea\label{ex:anderson:75}
 \ili{Tiv} \citep[\ili{Tiv} 4]{Arnott19671980}        \textsc{[Bantoid]}\\
...
\ili{Duka} \citep[17]{BendorSamuelEtAl1973}      \textsc{[Kainji]}\\
\gll  m\'ɛ    róà    sə  á\\
...
\ili{Berom}  \citep[300]{Bouquiaux1970}        \textsc{[Plateau]}\\
\gll mé    ciŋ\\
1.\textsc{sbjn}  dig\\
\glt `I must dig.'
\z
...
%@misc{Anderson2012,
%	author = {Anderson, Gregory D. S},
%	title = {S/TAM/P morphs in the history of \ili{Benue}-Congo and \ili{Niger-Congo} conjugation. Presented at Niger-Congress, Paris, September 2012.},
%	year = {2012}
%}
%
...
%	address = {Köln},
%	author = {Anderson, Gregory D. S},
%	booktitle = {\textit{\ili{Nilo-Saharan}: Models and descriptions},},
%	editor = {Mietzner, Angelika \& Storch, Anne},
%	pages = {151-167},
%	publisher = {Rüdiger Köppe Verlag},
%	title = {STAMP morphs in Central \ili{Sudanic} languages},
%	year = {2015}
%}
%
%@article{Arnott1958,
...
%}
%
%Arnott, D. W. 1967/1980. \ili{Tiv}. In Kropp-Dakubu, M. E. (ed.), \textit{West African language data sheets} \textit{volume 2}. 8 pp. Accra: West African Linguistic Society.
%
%@article{Babaev2010,
%	author = {Babaev, Kirill},
%	journal = {\textit{Journal of Language Relationship/Voprosy Jazykovogo Rodstva}},
%	pages = {1–45},
%	title = {Reconstructing \ili{Benue}-Congo person marking II},
%	volume = {4},
%	year = {2010}
%}
%
...
%	title = {\textit{\ili{Duka} sentence, clause and phrase}.},
%	year = {1973}
%}
%\end{styleHeader}
%
...
%	title = {\textit{A grammar of \ili{Ma'di}.} (Mouton Grammar Library 32)},
%	year = {2003}
%}
%
...
%	title = {\textit{La langue \ili{Birom} (Nigeria septentrional) –phonologie, morphologie, syntaxe},
%	year = {1970}
%}
%
...
%	title = {The pronoun system of some \ili{Chadic} languages},
%	year = {1986}
%}
%
%@article{Caron2001,
...
%	pages = {1-60},
%	title = {Dott, aka \ili{Zoɗi}, (\ili{Chadic}, West-B, South \ili{Bauchi}): Grammatical notes and vocabulary},
%	volume = {84},
%	year = {2001}
%}
%
...
%	journal = {\textit{Afrika und Übersee}},
%	pages = {161-248},
%	title = {Guus, aka \ili{Sigidi} (\ili{Chadic}, West-B, South \ili{Bauchi}): Grammatical notes, vocabulary and text},
%	volume = {85},
%	year = {2002}
%}
%
%Caron, B. 2006. South \ili{Bauchi} West pronominal and TAM systems. In Caron, B. \& Zima, P.~(eds.), \textit{Sprachbund in the West African Sahel,} 93-112\textit{.} Louvain/Paris: Peeters.
%
...
%	booktitle = {\textit{\ili{Bantu} in Bloomsbury: Special issue on \ili{Bantu} linguistics},},
%	editor = {Kula, Nancy C. \& Marten, Lutz},
%	pages = {187-200},
%	publisher = {University of London},
%	title = {Complex verb morphology in \ili{Kulango} (\ili{Gur}): Similarities and dissimilarities with Bantu},
%	year = {2007}
%}
%
%\begin{styleHeader}
...
%	author = {Èrman, Anna V},
%	booktitle = {\textit{\ili{Ju|’hoan}zhnye mande: Lingvistika afrikanskikh ritmakh. Materialy peterburgskoj èkspeditsii v Kot d'Ivuar (K 50-letiju Konstantina Pozdnjakov)},},
%	editor = {Vydrin, Valentin F. \& Zheltov, Aleksandr \ili{Ju|’hoan}.},
%	pages = {154-82},
%	publisher = {Evropejskij Dom},
%	title = {Sub''ektnye mestoimenija v dan-blovo I modal'no-aspektno-temporal'nye znachenija [Subject pronouns in Dan-\ili{Blowo} and (their) modal-aspectual-temporal meanings]},
%	year = {2002}
%}
%\end{styleHeader}
%
%Frajzyngier, Zygmunt. 1977. On the intransitive copy pronouns in \ili{Chadic}. \textit{Studies in African Linguistics, Supplement} VII. 73-84.
%
...
%	pages = {151-185},
%	publisher = {Cambridge University Press},
%	title = {The \ili{Macro-Sudan} belt: Towards identifying a linguistic area in northern sub-Saharan Africa},
%	year = {2008}
%}
%
%@book{Hodge1971,
...
%}
%
%Hofer, Verena \& Link, Christa. 1973/1980. \ili{Wobé}. In Kropp-Dakubu, M. E. (ed.), \textit{West African language data sheets} \textit{volume} \textit{2}. 8 pp. Accra: West African Linguistic Society.
%
%@book{Jones1998,
%	address = {Munich},
...
%	title = {\textit{The \ili{Boko/Busa} language cluster}.},
%	year = {1998}
%}
%
%@article{\ili{Ju|’hoan}ngraithmayr1968,
%	author = {\ili{Ju|’hoan}ngraithmayr, Herrmann},
%	journal = {\textit{Afrika und Übersee}},
%	pages = {161-204},
%	title = {/1969. The class languages of the \ili{Tangale-Waja} district},
%	volume = {52},
%	year = {1968}
%}
%
%\ili{Ju|’hoan}ngraithmayr, Herrmann. 2005. Prefix and suffix conjugation in \ili{Chadic}. \textit{Proceedings of the 10th Meeting of \ili{Hamito-Semitic} (Afroasiatic) Linguistics}, 411-419.
%
%\ili{Ju|’hoan}ngraithmayr, Herrmann. 2006. The verb in \ili{Chadic}–state of the art.~In Caron, B. \& Zima, P.~(eds.), \textit{Sprachbund in the West African Sahel,} 167-183. Louvain/Paris: Peeters.
%
%@book{Kim2002,
%	address = {Bamako},
...
%	title = {\textit{Die Sprache der \ili{Buduma} im Zentral Sudan}. \textit{Auf Grund eigener Studien und des Nachlasses von Gustav Nachtigal}},
%	year = {1939}
%}
%
...
%	author = {Moñino, Yves.},
%	publisher = {Peeters},
%	title = {\textit{Le \ili{Proto-Gbaya}.} \textit{Essai de linguistique comparative sur vingt-et-une langues d'Afrique centrale}},
%	year = {1995}
%}
%
%@book{Moser2004,
...
%	publisher = {LINCOM},
%	series = {LINCOM studies in African linguistics},
%	title = {\textit{Kabba: A \ili{Nilo-Saharan} language of the Central African Republic}.},
%	year = {2004}
%}
%
%Mukarovsky, Hans G.~1983. \textit{Pronouns and prefix conjugation in \ili{Chadic} and Hamito-Semitic}. Hamburg: Helmut Buske Verlag.
%
%@article{Nettle1998,
...
%}
%
%Pichl, Walter J. 1973/1980. Ndut-\ili{Falor}. In Kropp-Dakubu, M. E. (ed.), \textit{West African language data sheets} \textit{volume 2}, 8 pp. Accra: West African Linguistic Society.
%
%@book{Pohlig1992,
%	address = {Yaoundé},
%	author = {Pohlig, James N.},
%	publisher = {SIL},
%	title = {\textit{An account of \ili{Mofu-Gudur} verb infixes and suffixes}. (Ministry of Scientific and Technical Research)},
%	year = {1992}
%}
%
...
%}
%
%Schuh, Russell G.~1976. \textit{The \ili{Chadic} verbal system and its Afroasiatic nature}. Malibu, California: Undena Publication.
%
%@article{Seidel1898,
...
%	title = {\textit{The \ili{Zing} dialect of \ili{Mumuye}. A descriptive grammar. With a \ili{Mumuye}-\ili{English} dictionary and an \ili{English}-\ili{Mumuye} index}},
%	year = {1983}
%}
%
%@article{Sibomana1981,
%	author = {Sibomana, Leo},
%	journal = {\ili{Tarok} III: Das Verbalsystem und der Satz\textit{.} \textit{Afrika und Übersee}} ,
%	number = {2},
%	pages = {237–247},
%	title = {/82},
%	volume = {64},
...
%	publisher = {Oxford University Press},
%	title = {\textit{Linguistic analyses: The non-\ili{Bantu} languages of north-eastern Africa}},
%	year = {1966}
%}
%
%@book{Vandame1963,
...
%	publisher = {IFAN},
%	title = {\textit{Le \ili{Ngambay-Moundou}. Phonologie, grammaire et textes}},
%	year = {1963}
%}
%
%Voigt, Rainer M. 1989. Verbal conjugation in Proto-\ili{Chadic}.~In Frajzyngier, Zygmunt (ed.), \textit{Current progress in \ili{Chadic} linguistics: Proceedings of the international symposium on \ili{Chadic} linguistics, Boulder, Colorado, 1-2 May, 1987}, 267-284. (Vol. 62). Amsterdam: John Benjamins.
%
%@article{Voigt1987,
%	author = {Voigt, Rainer M},
%	journal = {The two prefix-conjugations in East Cushitic, East \ili{Semitic}, and \ili{Chadic}.~\textit{Bulletin of the School of Oriental and African Studies}~},
%	number = {2},
%	pages = {330-345},
%	volume = {50},
%	year = {1987}
