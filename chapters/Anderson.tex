...
\z
  ...
  In this study, I discuss some of the characteristics of STAMP morphs and STAMP morph constructions in a set of languages and genetic units of the Macro-Sudan Belt. \sectref{sec:2} discusses the genetic units that constitute the typological sampling of African languages used in this study. \sectref{sec:3} discusses the range of functions associated with STAMP morph constructions, and \sectref{sec:4} explores their formal constructional features. \sectref{sec:5} offers some thoughts on the origins of STAMP morphs.  \sectref{sec:6} discusses the subsequent role such formations have played in the development of prefixed conjugation series that are likewise found across the different genetic units of this linguistic area. Sections 7-9 briefly examine the role of STAMP morph constructions and prefix conjugations that developed out of erstwhile STAMP morphs in three macro-level groupings of genetic units (stocks) in the Macro-Sudan Belt. This includes the Chadic stock (\sectref{sec:7}), the Central Sudanic stock of Nilo-Saharan (\sectref{sec:8}), and the various genetic units found within the Benue-Congo stock of Niger-Congo (\sectref{sec:9}).

\section{Typological Sampling and the Macro-Sudan Belt}\label{sec:2}

In his seminal work on the Macro-Sudan Belt [MSB], \cite{G\"uldemann2008} remained agnostic about the usefulness of higher order units in Africa, at least with respect to typological sampling. Instead he suggested the fourteen genera in \tabref{tab:1}, with Ijoid, Dogon, Chadic and Songhay being peripheral to the proposed area insofar as they show fewer of the characteristic core-features identified as definitional of the MSB, than do the majority of other identified genera.

\begin{table}
\label{tab:1} 
\caption{Genera in the Macro-Sudan Belt according to \cite{G\"uldemann2008}} 
\begin{tabularx}{\textwidth}{XXXX}
\lsptoprule
\mdseries Adamawa & \mdseries non-Bantu Benue Congo & \mdseries Bongo-Bagirmi & \mdseries Dogon\\
\mdseries Kwa & \mdseries Kru & \mdseries Gur & \mdseries Songhay\\
\mdseries Mande & \mdseries Moru-Mangbetu & \mdseries Ubangian & \mdseries Ijoid\\
\mdseries Chadic & \mdseries Atlantic &  & \\
\lspbottomrule
\end{tabularx}
\end{table} 

Since 2008, a number of revisions to standard classifications of African languages have been offered, including statements by \cite{Dimmendaal2008, Dimmendaal2011} and \citet{Sands2009}. In previous work on African linguistic typology, I have suggested a larger range of sampling units as appropriate for cross-linguistic work, which have internal diversification on the level of Germanic or Romance; I call these `genetic units for typological sampling' \citep{Anderson2011}, and write them in small-caps to distinguish them from names of both languages and larger taxa. Many genera in \tabref{tab:1} would thus be split into multiple different genetic units; in this system, the number of such genetic units in the Macro-Sudan Belt to use in typological sampling totals over fifty. Not all these genetic units have attested STAMP morph constructions, but the vast majority does. In my corpus the genetic units that include STAMP morph constructions (or prefix conjugation series that historically derive from such constructions, see section 5) include at least those in \tabref{tab:2}.
...
\section{Functions of STAMP morphs}\label{sec:3}
...
Wob\'{e} (\citealt{Hofer&Link:1973}: Wobé 3)\footnote{Hofer \& Link's volume does not number the chapters consecutively, but rather each chapter begins anew at page 1. Thus this form comes from page 3 of the Wobé chapter. Their page-numbering convention is followed here.}    \textsc{[Kru]}\\
\gll ẽ\textsuperscript{ 2}    gyi\textsuperscript{32}            \\
1.\textsc{pst}  come  \\
\glt `I have come.'            
\z
\ea \label{ex:anderson:4b}
Wobé\\
...
As noted earlier, the referent categories expressed in STAMP morphs are typically restricted to subjects, but in a small number of instances one also finds portmanteau subject {\textgreater} object forms within a STAMP morph, as in the following sentence from Kohumono (\textsc{Cross River}).

\ea\label{ex:anderson:8}
 Kohumono \citep[355]{Cook1972}         [\textsc{Cross River}]\\
\gll $\beta ɔ$    fà\\
1{\textgreater}2.\textsc{npst}  bite\\
\glt `I bite you.'
\z

In Guro (\textsc{Southeast Mande}), one finds portmanteau subject {\textgreater} object-encoding STAMP morphs that also reference polarity, as in \textit{ɓ}\textit{e} vs. \textit{yaa} in the following sentences (note the tone differences on the verbs in these constructions as well as the presence of the postverbal negator \textit{ɗ}\textit{o} in \REF{ex:anderson:9b}). 

\ea\label{ex:anderson:9}
\ea\label{ex:anderson:9a}
Guro \citep[239]{Vydrine2009}       [\textsc{Southeast}\textsc{ Mande}]\\
\gll  ɓe      zuru-o      \\
  \textsc{2sg{\textgreater}3sg.ipfv}    wash-\textsc{ipfv} \\
\glt `[You] wash him/her/it.'       

\ex \label{ex:anderson:9b}
\gll  yaa      zùrù-ò    ɗo\\
  \textsc{2sg{\textgreater}3sg.ipfv.neg}  wash-\textsc{ipfv}  \textsc{neg}\\
\glt `[You] don't wash him/her/it.'
\z
\z

\sectref{sec:4} now turns to the range of constructional types in which STAMP morphs participate.


\section{Formal subtypes of STAMP morph constructions}\label{sec:4}
...
\ea\label{ex:anderson:12}
 STAMP + Floating Tone Construction:   STAMP Verb\textsubscript{{\textless}construction-specific tonal form{\textgreater}  }
 \z

West Chadic Guus (also known as Sigidi) also has a highly elaborated system of STAMP morphs (\tabref{tab:3}), some of which have associated floating tones that they project onto the verb. The Recent Past (\textsc{rec) }form projects a high tone onto the following verb. The Future (\textsc{fut) }does the same, and differs from the otherwise identical-looking subjunctive [\textsc{sbjn] }in whether it assigns this floating high tone (\textsc{fut}), or does not (\textsc{sbjn}); the two are thus \textit{constructionally }distinct.
...
\gll má     dɔlɪ  \\
\textsc{  I.hab}\textsubscript{{\textless}+L{\textgreater}}  sell\\        
\glt `I sell.'            

\ex \label{ex:anderson:14b}
\gll mɪ    dɔlɪ\\
  1.\textsc{sbjn}\textsubscript{{\textless}+H{\textgreater}}  sell\\
\glt 'May I sell.'
\z
\z
...
Aspect Construction:    STAMP  Verb\textsubscript{{\textless}}\textsc{\textsubscript{asp{\textgreater}}}\textsc{    }
...
\ea\label{ex:anderson:21}
  Tonal Morphology  STAMP   Verb\textsc{\textsubscript{{\textless}Asp=Tone{\textgreater}}}
\z
...
Modal Construction  STAMP  Verb\textsubscript{{\textless}}\textsc{\textsubscript{modal}}\textsubscript{{\textgreater}}\\
\z
...
Split Negative Construction:  \textsc{STAMP  V}erb\textsc{\textsubscript{{\textless}neg{\textgreater}}}
\z
...
Split Object Construction:   STAMP  Verb\textsc{\textsubscript{{\textless}obj{\textgreater}}}
\z
...
Garba  \textsc{cop}  cut  meat  \textsc{acc  inj  2:aor  }say-1  \\
...
\section{Origin of STAMP morphs from auxiliary constructions}\label{sec:5}
...
\gll mɛ:\textsuperscript{32  }wɨ\textsuperscript{3  } {\textasciitilde}   mɛ\textsuperscript{3}  ʃe\textsuperscript{2}  wɨ\textsuperscript{3    }mɛ:\textsuperscript{32 }{\textless} mɛ\textsuperscript{3}=ʃe\textsuperscript{2}\\
...
\section{STAMP morphs developing into prefixal conjugations}\label{sec:6}
...
\section{STAMP morphs and prefix conjugations in Chadic languages}\label{sec:7}
...
Angas  \citep[38]{Burquest1973/1980}         [\textsc{West Chadic]}\\
\gll ŋâː    jì\\
1.\textsc{compl}  come\\
\glt  `I have come.'
\z
...
Gerka  \citep[173]{Jungraithmayr1968/1969}
\gll kà  tà  ƒàm\\
...
\section{STAMP morphs and conjugational prefixes in Central Sudanic languages}\label{sec:8}
...
m\textcyrillic{а}\={ }   k-ào   àl   ngà\\
   1.\textsc{fut}   \textsc{inf}{}-go  \textsc{neg}   \textsc{adv}\\
\glt `I will not go again.'

\ex \label{ex:anderson:48b}
\gll á   k-ùs\textcyrillic{а}\={ }   né   ngà   uà\\
\textit{  }2.\textsc{fut}   \textsc{inf}{}-eat thing   \textsc{adv   q}\\
...
1:\textsc{fut}  \textsc{inf}{}-go  place  \textsc{loc  neg}    \\
...
Bongo   \textit{*mi- {\textless} m-i} \citep[75]{Tucker&Bryan1966}    [\textsc{Sara-Bongo-Bagirmi}]\\
...
\gll má    y-ane    yi\\
\textit{  }1.\textsc{indef}  \textsc{inf}{}-eat    thing\\
...
\gll ni-$\text{\textgreek{'e}}$si-(a)      \\
2.\textsc{prs/fut}{}-do-\textsc{tam}        \\
\glt `You (will) do.' 
\z

\ea\label{ex:anderson:61}
Meje \citep[106]{Larochette1958}\textit{ }          \textsc{[Mangbetu]}\\
\gll ni-$\text{\textgreek{'e}}$sí-a\\
...
\section{STAMP morphs and conjugational prefixes in Niger-Congo languages}\label{sec:9}
...
%
%Jungraithmayr, Herrmann. 2006. The verb in Chadic–state of the art.~In Caron, B. \& Zima, P.~(eds.), \textit{Sprachbund in the West African Sahel, }167-183.\textit{ }Louvain/Paris: Peeters.
%
%@book{Kim2002,
%	address = {Bamako},
...
%	booktitle = {\textit{Negation Patterns in West African}\textit{ }\textit{Languages},},
