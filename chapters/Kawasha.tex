...
Compare the examples in \tabref{tab:2.kawasha} and \tabref{tab:3.kawasha}. Verb roots with non-mid vowels in \tabref{tab:2.kawasha} have an inflectional suffix with an initial high front vowel /i/, while those with mid vowels in \tabref{tab:3.kawasha} contain a suffix with an initial mi vowel /e/. 

\begin{table}
\begin{tabularx}{\textwidth}{llXXl}%final column can be removed
\lsptoprule
...
Kaonde, like a many other Bantu languages \citep{greenberg1951}, presents partial nasal consonant harmony that operates on suffixes if a verb root ends in a non-palatal or non-velar nasal /m/ or /n/. This process assimilates the segment /l/ of the suffix to an alveolar nasal by harmonizing it with the preceding nasal segment of the root in a single feature. That is, the base nasal triggers left-to-right harmony targeting the lateral consonant in suffix.\footnote{Unlike some Bantu languages such as Kikongo, Kiyaka, Chokwe, etc. nasalization does not operate in Kaonde if the input in NVC-\textit{ile} or CVNVC-\textit{ile}, where the final consonant of the verb root is not a nasal.} Notice that nasal consonants induce nasalization of underlying /l/ of the remote past suffix in \tabref{tab:4.kawasha}. 
...
This same process also affects a set of monosyllabic verb roots ending in [l]. The root final consonant palatalizes to \textit{zh} when the causative suffix -\textit{ish} is added.\footnote{The data in \tabref{tab:6.kawasha} involve imbrication, discussed in \sectref{§4:imbrication.kawasha}. The type of imbrication that turns the voiceless consonant \textit{sh} of the causative suffix into a voiced one is also attested in Chiluba \citep[73]{lukusa1993}.} Compare the underived verb stem in the first column of \tabref{tab:6.kawasha} with their causativized verb forms in the second column.
...
The /l/ of the remote past suffix assimilates completely to the causative consonant when the two suffixes occur together (\tabref{tab:7.kawasha}).
...
The coronal voiceless stop /t/ changes in manner of articulation to become an alveopalatal affricate /tʃ/. Thus, verb root/base final consonants alternate between [+cor, +ant] and [+cor, -ant] (\tabref{tab:10.kawasha}).
...
Also susceptible to palatalization are verb roots/bases that end in the lateral /l/ which becomes a voiced alveopalatal affricate /dʒ/ before \textit{-ile}. In short, this consonant, which varies between [l] and a central flap [ɾ] or a lateral flap [ɺ], alternates with /dʒ/. Consider mutation of the verb final consonant /l/ to the alveopalatal affricate /dʒ/ in \tabref{tab:12.kawasha}.\footnote{In Lunda the cognates to \textit{shaala} ‘remain’ and t\textit{waala }‘take’ in \tabref{tab:12.kawasha} undergo imbrication to become \textit{sheeli} and \textit{tweeli} respectively.}
...
The applicative suffix -\textit{il} changes to -\textit{ij} when it precedes the remote past suffix.\footnote{Like the remote past, the causative, applicative, and perfective suffixes also condition palatalization. For example, based on the root \textit{kas} ‘tie’, Kaonde has the intensive form \textit{kash-ish-a} ‘tie securely’, \textit{kash-il-a} ‘tie for’, and \textit{kash-ij-il-a} ‘tie completely for’.} This is illustrated by the examples in the right column of \tabref{tab:13.kawasha} where the inflectional remote past morpheme follows the applicative. The last example in \tabref{tab:13.kawasha} possesses two palatalized segments, i.e. the root final consonant /ʃ/ and /dʒ/ of the applicative extension. 
...
\midrule
 & \textit{ketekel-a} & `trust' & \textit{ketekej-ile} & `trusted' \\
...
\begin{table}
\fittable{
\begin{tabular}{lp{3.5cm}lp{2.1cm}l}
\lsptoprule
