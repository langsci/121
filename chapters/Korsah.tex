...
Unlike languages like English however,  Akan (Kwa, Niger-Congo) does not permit gaps in any extraction site. As \REF{ex:korsah:2} shows, the RP is always obligatory.\footnote{The data presented here is based on the intuitions of three native speakers. Akan has both high and low tones. But I will mark only high tones.} 
...
\begin{table}[h]
  ...
  \lsptoprule	
  		...
  		& 2 & $wo$-/\textit{i} & $wo$/-$w$ \\
  		%   \hline
  		&  3(\textsc{anim}) & \textit{\oor-}/\textit{no} & $no$/-$n$\\
  		
  		&  3(\textsc{inanim}) & \textit{\eer-}\textit{no}/ \textit{\oor-}\textit{no} & ($no$/-$n$) \\
  		%\hline
  		%\textsc{pl} &   & & \\
  		%\hline
  		\textsc{pl} & 1 &  \textit{y\eer-} & \textit{y\eer n}/\textit{h\eer n}\\
  		%    \hline
  		& 2 & \textit{mo-/hom-} & \textit{mo/hom} \\
  		%   \hline
  		&  3(\textsc{anim}) & \textit{w\oor-}& \textit{w\oor n/h\oor n}\\
  		
  		&  3(\textsc{inanim}) & \textit{\eer-}($no$)/\textit{\oor-}($no$)& \textit{w\oor n/h\oor n} \\
  	 \lsptoprule
  	\end{tabular}
...
For object extraction, resumption is  obligatory for only animate NPs.  For instance,  the RP \textit{n\h o} in \REF{ex:korsah:7} is obligatory. For inanimate object NPs, however, the overt realization of an RP appears to be optional; gaps are sometimes permitted, as in when \REF{ex:korsah:8a} and \REF{ex:korsah:8b} are compared. This distribution of gaps and RPs with regards to extracted object NPs inanimate in Akan, may be attributable to independent properties of either the pronoun system, or the verbs involved (see e.g.\ \citealt{Chinebuah76} and \citealt{Larson05}  properties in other Kwa languages).
...
\noindent i. \textsc{person}: Participant (i.e.\ 1st/2nd person) or Non-participant (i.e.\ 3rd person).

\noindent ii. \textsc{number}: Singular (Sg) or Plural (Pl)
 
\noindent iii. \textsc{animacy}: Animate or Inanimate (Inan)
...
The distribution of (agreeing) resumptive pronouns and the (non-agreeing) default pronoun in Akan can be summarize  as presented in \tabref{tab:korsah:3}.
