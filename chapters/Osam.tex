...
\gll Wɔ̀-dè    àsɛ́ḿ  nó  kɔ́-ɔ̀    àhèmfíé.\\
...
Even though in the constructions in \EF{ex:35.osam} and \REF{ex:36.osam} there are two arguments, A and O, functionally, there is reduced valency because the subjects are not prototypical subjects. The sentences have \textsc{agent} subjects. Nonetheless, the \textsc{agents} involved are not distinct from the \textsc{patients} because the pronominal form used is non-referential. This makes it impossible to identify the referent, an obvious way of downgrading the \textsc{agent} argument.
...
\ea\label{ex:42.osam}
\ea\label{ex:42a.osam}\it{Casuative} \\
...
\ex\label{ex:42b.osam}\it{Anticausative}\\
