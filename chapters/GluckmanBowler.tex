...
\ea\label{ex:gluckman:1a} 
   John broke the vase.~ ~ ~ ~ ~       (lexical) causative \\
 \ex \label{ex:gluckman:1b} 
 The vase broke. ~ ~ ~ ~ ~ ~       anticausative \\
 \z
\z

\ea\label{ex:gluckman:2}
\ea \label{ex:gluckman:2a}
 {Luragooli}{}{}\\
\gll  Sira a-han-i       muriaŋgo.~ ~ ~ ~     (lexical) causative\\
       Sira 1-close-\textsc{fv} 3door\\
\glt ‘Sira closed the door.’
\ex\label{ex:gluckman:2b}
\gll  muriaŋgo gu-han-ik-i.~ ~ ~ ~ ~     anticausative\\
...
\ea\label{ex:gluckman:4} 
\ea{
\settowidth\jamwidth{causative}
{Causative}\\
\gll Sira    a-han-i      muriaŋgo.      \\
     1Sira 1-close-\textsc{fv}  3door\\\jambox{causative}
}
\glt ‘Sira closed the door.’
...
\todo[inline]{some examples have headings above, others have `comments' on the left margin. A uniform style should be chosen}
\ea\label{ex:gluckman:5} 
{Anticausative}\\
\ea{\label{ex:gluckman:5a} 
\settowidth\jamwidth{plain intransitive}
\gll muriaŋgo gu-han-i       \\
       3door       3-close-\textsc{fv}\\ \jambox{plain intransitive}
}
\glt ‘The door closed.’
\ex{\label{ex:gluckman:5b} 
\settowidth\jamwidth{intransitive}
\gll muriaŋgo gu-han-ik-i.      -\textup{Vk} \\
       3door         3-close-\textsc{Vk}-\textsc{fv}\\\jambox{intransitive}
}
...
Thus, in terms of oblique theta-roles, the plain intransitive and the \textit{-Vk} intransitive pattern together, separately from the Luragooli passive. This is summarized in \tabref{tab:gluckman:2}.
...
The Luragooli patterns in \tabref{tab:gluckman:2} largely parallel properties of anticausative versus passive constructions in other languages. The anticausative forms do not permit oblique \textsc{agents} or \textsc{instruments}, but are compatible with oblique \textsc{causers.}

\section{Lexical aspect}\label{sec:gluckman:4}

In \sectref{sec:gluckman:3} we demonstrated how both plain and \textit{-Vk} intransitives are distinct from the passive. In this section, we will show how \textit{-Vk} intransitives are distinct from the plain intransitive and the passive. The data in \sectref{sec:gluckman:4} concerns lexical aspect, or Aktionsart. We use four pieces of evidence to show that \textit{-Vk} intransitives differ from the other two intransitive forms with respect to lexical aspect. Our evidence involves interaction with negation, complementation under ‘want’, progressive aspect, and continuations. Our data suggest the provisional generalization in \REF{ex:gluckman:10}, which we will revise in \sectref{sec:gluckman:4.5}.\todo{There is currently no section 4.5. Or is "subclasses of class I verbs a subsection?}
