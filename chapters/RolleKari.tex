...
\end{table}
\todo{Table 1 needs to restore missing "3rd" "Set 2" column, as in the original manuscript. Also center "1st", "2nd", "3rd" between each of the respective "Set 1" and "Set 2" columns.}
...
\end{tabularx}
\end{table}
\todo{Author wants line "& \multirow{4}{*}{V=\textbf{\textsc{cl} XP}}" to be vertically centered in the blank space between the "SG" and "PL" blocks; but horizontally to the right where it is. (Also, can "SG" and "PL" row headings can be centered vertically between the first and second, and third and fourth rows, respectively - aesthetic issue.) }
...
\todo[inline]{Should the shading extend to column 3?}
\label{table:rolle:3}
...
\midrule
{Factative}           & {o=\'V=n}    &\shadecell {o=\'V \'V=n}      &\shadecell {o=\'V $\sigma $ \'V=n} & {o=\'V=n $\sigma $  $\sigma $ o=\'V=n} \\
\tablevspace
{Perfect}             & {o=\'{V}=tē} &\shadecell {o=\'{V} \'{V}=tē} &\shadecell {o=\'{V} $\sigma $ \'{V}=tē} & {o=\'V=t\=e $\sigma $ $\sigma $ o=\'V=t\=e}\\
\tablevspace
{Present/ Habitual}   & {mó=\'{V}}   & {o=\'{V} mō=\'{V}} & {o=\'{V} $\sigma $ mō=\'{V}} & {o=\'V $\sigma $ $\sigma $ m\'o=\'V}\\
\tablevspace
{Future}              & {mó=\'{V}}   &\shadecell {mó=\'{V} \'{V}}   &\shadecell {mó=\'{V} $\sigma $ \'{V}} & {m\'o=\'V $\sigma $ $\sigma $ m\'o=\'V}\\
...
\gll   \textsuperscript{?}Breno  \textbf{mó}=ḍúw    mé   \textbf{m\'{ọ}}=tá    \\
...
\label{table:rolle:4}
\end{table}
\todo{Table 4 is missing the last column from the original. Please add.}
...
\todo{{"t"} in first ex line needs raised IPA downarrow before it}  
...
{\db}N      ~             {\db}V       ~     ~             \textbf{σσ}  ~ ~         {\db}V  ~\\
 {\db}Breno ~             {\db}ḍuw     ~     ~             \textbf{ọyi} ~ ~         {\db}tạ ~\\
\z
\z
...
In \REF{ex:rolle:53}\todo{Please check if this links to the correct example!}, this HL negation tone also applies when the verb appears with another verb or a prosodically light pronoun, both Single-Marked contexts. Here, all elements in the complex are marked with low tone. We can compare this with Double-Marked contexts when there is a prosodically heavy intervening object. In \REF{ex:rolle:53}\todo{Please check if this links to the correct example!}, the first verb and proclitic \textit{vọn} ‘take’ is marked with the HL negation pattern, whereas the second verb \textit{fịya} ‘cut’ is marked with H, outside of the scope of negative tone. Example \REF{ex:rolle:54} illustrates that marking both verbs with HL negation is ungrammatical. 
