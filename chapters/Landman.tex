...
\todo{Presumably, some sections should be subsections}
...
\lspbottomrule
\end{tabularx}
\todo[inline]{somethings seems to have gone wrong with this table}
\end{table}
...
\tabref{tab:2} provides a summary of the differences between -\textit{oosi} and \textit{vuri}. In brief, their semantic properties accord with familiar differences between non-distributive and distributive quantifiers. Syntactically speaking, we see a divide that will factor into the analysis developed below.
...
\z
\todo{three opening brackets, only two closing brackets}
