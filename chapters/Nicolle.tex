...
\begin{table}
\begin{tabularx}{\textwidth}{p{1.5cm}p{1cm}XXp{1.8cm}}
\lsptoprule
Language & Classification\footnote{Languages are classified using the ISO 639-3 language code as cited in \textit{Ethnologue} \citep{lewisetal2015} and \citeauthor{maho2003}’s (\citeyear{maho2003}) updated version of \citeauthor{guthrie1967}’s (\citeyear{guthrie1967}) classification of the Bantu languages.}
...
In other languages, temporary topics are left dislocated. In the Bena example in \ref{ex:15.nicolle},  \textit{inyama} ‘meat’ is a left dislocated temporary topic; \textit{u}\textit{-Mbwa} ‘Dog’ occurs after the verb because it receives argument focus (see \sectref{§4:expression.nicolle}).
