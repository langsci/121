...
\ea\label{ex:nguna:4} {Hebrew \citep[1]{Danon2002}}\\
\ea\label{ex:nguna:}
\gll Dan         kara                 sefer\\
     Dan         read                  book\\
\glt ‘Dan reads a book.’
...
\section{DOM realization in Shimakonde and Emakhuwa}\label{sec:nguna:3}
...
\ea\label{ex:nguna:5} {Emakhuwa (human object)}\\
...
\begin{table}
\caption{Shimakonde: [-\textsc{human, -animate}] nouns of classes 5, 7, and 9}
\label{tab:nguna:1}
...
\caption{Shimakonde: [\textsc{-human}, \textsc{+animate}] nouns of classes 5, 7, and 9}
\label{tab:nguna:2}
...
\footnotetext{According to \citet{Duarte2011}, the particle \textit{a} occurs only in some dialectical varieties of Changana and Rhonga. When it precedes a DP object, its major role is to encode definiteness and to indicate that the referent of that object has already been given in previous discourse. } 
...
Given these background assumptions, we will assume, hereafter, that Rhonga and Changana set “yes” to Baker’s Directionality of Agreement Parameter \REF{ex:nguna:23}. Notice that “F” can be read as the little \textit{v}\textsuperscript{o} that heads the {\textit{v}}P projection in \figref{fig:nguna:2}.
